...
begin
    ...
    function plot_two(sol)
        ...
        scatter(x, y; label = "y")
        plot!(f, 1, 26; label = "fitted curve")
        plot!(x, y; label = "linear interpolation", ls = :dash)
        plot!(
            x,
            interpolate(y, BSpline(Quadratic(Reflect(OnGrid()))));
...
begin
    ...
    function solve_three(enable_disturbance_mode)
        ...
        model(x, p) = sum([p[i] * x .^ (i - 1) for i = 1:length(p)])
        p0 = zeros(10)
    ...
    function plot_three(sol)
        ...
        scatter(y; label = "y", ylims = (20, 80))
        plot!(lsq_model, label = "OLS")
        plot!(lasso_model, label = "LASSO")
        plot!(ridge_model, label = "ridge")
        plot!(elastic_model; label = "elastic net", ls = :dash)
    end
end
