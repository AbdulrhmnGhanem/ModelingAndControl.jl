...
begin
    ...
    sol_analytical_1 = tspan_to_range(tspan_1, h_1_b / 2, collect = true),
    analytical_f.(tspan_to_range(tspan_1, h_1_b / 2))
...
sol_2_tist = @btime solve(prob_2, Tsit5(); reltol = 1e-3, abstol = 1e-5);

# ╔═╡ 10dc5bce-7a41-4158-9460-408025db4694
md"#### Stiff solver"

# ╔═╡ 635aff41-8939-485c-906e-c28a849ae33c
sol_2_rodas = @btime solve(prob_2, Rodas4(); reltol = 1e-3, abstol = 1e-5);
...
begin
    ...
    plot!(sol_2_rodas[1:16]; label = "Rodas4 (stiff)", ls = :dash)
end
...
begin
    ...
    sol_3_dp5 = solve(prob_3, DP5(); reltol = 1e-3, abstol = 1e-5)

    sol_3_vcabm = solve(prob_3, VCABM(); reltol = 1e-3, abstol = 1e-5)

    sol_3_qndf = solve(prob_3, QNDF(); reltol = 1e-9, abstol = 1e-9)
end;
...
sol_4_tist = @btime solve(prob_4, Tsit5(); reltol = 1e-6, abstol = 1e-8);
...
sol_6_rodas = @btime solve(prob_6, Rodas4(); reltol = 1e-6, abstol = 1e-8);

# ╔═╡ 4a913865-b84d-4d2b-9168-ddd0d93f04ab
begin
    input(t) = abs(sin(333t))
    ddf_6_2 =
        second_order.@second_order (du, u, t) -> @. (10^4) * input(t) - 100du - (10^4)u

    h = 10e-5
    sol_6_euler =
        @btime second_order.euler_2nd_order(ddf_6_2, h, tspan_6, f0_6[1], df0_6[1])
...
begin
    ...
    plot!(inputs; xlims = (0, 0.1), label = "input")
end
