...
begin
    ...
    sol_analytical_2 =
        collect(tspan[1]:h_0:tspan[2]), f_2_anaytical.(collect(tspan[1]:h_0:tspan[2]))
...
begin
    ...
    plot!(sol_euler_2_a; label = "Euler (h=$h_2_a)", ls = :dot)
    plot!(sol_euler_2_b; label = "Euler (h=$h_2_b)", ls = :dash)
end
...
begin
    ...
    sol_analytical_3 = collect(tspan_3[1]:h_0:tspan_3[2]),
    f_3_anaytical.(collect(tspan_3[1]:h_0:tspan_3[2]))
...
begin
    ...
    plot!(sol_euler_3; label = "Euler", ls = :dot)
end
...
begin
    ...
    sol_analytical_4 = collect(tspan_4[1]:h_0:tspan_4[2]),
    f_4_anaytical.(collect(tspan_4[1]:h_0:tspan_4[2]))
...
begin
    ...
    plot!(sol_euler_4; label = "Euler", ls = :dot)
end
...
begin
    ...
    plot!(sol_euler_5_b; label = "h=$h_5_b")
    plot!(sol_euler_5_c; label = "h=$h_5_c")
    plot!(sol_euler_5_d; label = "h=$h_5_d")
end
...
begin
    ...
    plot!(sol_rk_6_b; label = "h=$h_6_b", ls = :dot)
    plot!(sol_rk_6_c; label = "h=$h_6_c", ls = :dash)
    plot!(sol_rk_6_d; label = "h=$h_6_d", ls = :dashdot)
end
...
begin
    ...
    sol_analytical_7 = collect(tspan_7[1]:h_0:tspan_7[2]),
    f_7_anaytical.(collect(tspan_7[1]:h_0:tspan_7[2]))
...
begin
    ...
    plot!(sol_euler_7; ls = :dot, label = "Euler")

    plot!(sol_analytical_7; label = "Analytical", ls = :dash)
end
...
begin
    ...
    plot!(sol_euler_8; ls = :dot, label = "Euler")
end
...
begin
    ...
    sol_analytical_9 = collect(tspan_9[1]:0.0001:tspan_9[2]),
    f_9_anaytical.(collect(tspan_9[1]:0.0001:tspan_9[2]))
...
begin
    ...
    plot!(sol_analytical_9; ls = :dash, label = "Analytcial")

    plot!(sol_euler_9; ls = :dot, label = "Euler")
end
...
function adams_multon_2nd_order(f, h, tspan, f0, f1; steps = 4)
    ...
    if !(steps in allowed_steps)
        throw(
            error(
                "Invalid step size for Adams-Multon! Allowed steps sizes are $allowed_steps.",
            ),
        )
    end
    ...
    for i in range(1, steps_num - lookahead)
        ...
        y_[i+3] =
            y_[i+2] +
            h * ((3 / 8) * k4() + (19 / 24) * k3() - (5 / 24) * k2() + (1 / 24) * k1())
    end
...
begin
    ...
    plot!(sol_am_12_2; label = "steps=2", ls = :dot)

    plot!(sol_am_12_3; label = "steps=3", ls = :dashdot)

    plot!(sol_am_12_4; label = "steps=4")
end
