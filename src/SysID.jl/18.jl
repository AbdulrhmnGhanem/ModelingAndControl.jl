...
md"# Exercise 18: Generate a sine wave, doubled measurement time

!!! purpose
	No leakage is observed if the we measure integer number of periods.

	- The line number changed: the excited frequency line is now 2 because two periods are now measured.

	- If the frequency axis is labeled with the DFT-line number, then the position changes with a varying window length.
"
