% Chapter 2 Exercise 22b
% Spectral analysis of a multisine, no leakage present
%
% Copyright: 
...
Fraction=0;                  % fraction of the uncompletely measured period
...
% DG_MakePDF('FigMultisinus3a.pdf', gcf);

FigNum=9;           % zoom of the FFT
figure(FigNum)
clf
...
subplot(1,2,2)
plot(f(LinesPlot),db(URect(LinesPlot)),'+k',f(LinesPlot),db(2*UHann(LinesPlot)),'.k')
axis([240 260 -10 20])
DG_SetFontSize(12)
DG_SetTraceWidth(0.5,1,FigNum)
DG_SetTraceWidth(1,2,FigNum)
...
% Export the plot
% DG_Init4PDF(gcf, 4.5);        % fixing the size, half standard height
% DG_MakePDF('FigMultisinus3b.pdf', gcf);
